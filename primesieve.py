def primes():
	...
	def wheel_prime_pairs():
		...
		sieve = {}; threadnum = 23; ni = 1  #   into sieve dict; init cndidate, wheel ndx
		while True:
			if threadnum not in sieve:  # is not a multiple of previously recorded primes
				if threadnum < q: yield (threadnum, ni)  # threadnum is prime with wheel modulo index 
				else:
					npi = pi + 1  # advance wheel index
					if npi > 92159: npi = 0
					sieve[q + p * gaps[pi]] = (p, npi)  # threadnum == p * p: put next cull position on wheel
					p, pi = next(bps); q = p * p  # advance next prime and prime square to put
			else:
				s, si = sieve.pop(threadnum)
				nxt = threadnum + s * gaps[si]  # move current cull position up the wheel
			...
			if nni > 92159: nni = 0
			threadnum += gaps[ni]; ni = nni  # advance on the wheel
